...
def _seg_into_senteces(doc):
    sentences = sent_tokenize(doc)
    return sentences


def _seg_into_words(doc):
    words = []
    for d in doc:
...
def _doc_frequency(_terms_of_all, _docterms):  # docterms = 20, terms of all = 1194
    df = []
    #dfd = []
...
def _tf_idf(_log_terms, _log_freq, _idf, _docterms, _final_terms):
    bydocTFIDF =[]
